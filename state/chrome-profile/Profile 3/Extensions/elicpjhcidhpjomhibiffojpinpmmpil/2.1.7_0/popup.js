...
function OnUltimate(mode) {
	if (mode == "protected")
		...
		chrome.tabs.create({ "url": "https://videodownloaderultimate.com/?p=spotify", "selected": true }, function (tab) { });
	else
...
function showVideoUrls() {
	...
	if (videoUrls) {
		...
		for (var idx = 0; idx < videoUrls.length; idx++) {
			...
			if (ob.noDL == "m3u8" || ob.noDL == "m4s" || ob.noDL == "spotify") {
				if (hasM3u)
			...
			else if (ob.noDL == "spotify") {
				var o2 = co('div', item, { 'class': 'clNODownloadButton clNODownloadButtonSpotify' });
				co('img', o2, { 'width': '19px', 'src': './png/no.png' });
			}
	...
	aElements = document.getElementsByClassName("clNODownloadButtonSpotify");
	for (var idx = 0; idx < aElements.length; idx++) {
		aElements[idx].addEventListener('click', function () { OnUltimate("spotify") });
	}
